...
PhyParam_t RegionCN470GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_CHANNELS_DEFAULT_MASK:
        {
            ...
            phyParam.ChannelsMask = lora_dev_para.freqBand;
            break;
        }
...
void RegionCN470InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
            ...
            memcpy(ChannelsMask,lora_dev_para.freqBand,sizeof(lora_dev_para.freqBand));
           // RegionCommonChanMaskCopy( ChannelsMask, lora_dev_para.freqBand, 6 );
            break;
        }
...
bool RegionCN470ChanMaskSet( ChanMaskSetParams_t* chanMaskSet )
{
    switch( chanMaskSet->ChannelsMaskType )
    {
        ...
        {
            RegionCommonChanMaskCopy( lora_dev_para.freqBand, chanMaskSet->ChannelsMaskIn, 6 );
            break;
        }
        default:
            return false;
...
bool RegionCN470RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
	...
	uint8_t tempChannel = rxConfig->Channel;
    ...
    { 
        if( 1 == lora_dev_para.certifi )
        {
            if(ULDL_MODE_INTER == lora_dev_para.band)
            {
				if(tempChannel >= 166)
				{
					tempChannel -= 66;
    		...
    		frequency = CN470_FIRST_TX_CHANNEL + tempChannel *
					CN470_STEPWIDTH_RX1_CHANNEL;
		}
		else
		{
			if( ULDL_MODE_INTRA == lora_dev_para.band )
			{
				frequency = CN470_FIRST_TX_CHANNEL + rxConfig->Channel *
					CN470_STEPWIDTH_RX1_CHANNEL;
			}
			else
			{
				frequency = CN470_FIRST_RX1_CHANNEL + ( rxConfig->Channel % 48 ) *
					CN470_STEPWIDTH_RX1_CHANNEL;
			}
	...
	else
    {
    	...
    	if(1 == lora_dev_para.certifi)
    	{
    		if(  ULDL_MODE_INTRA == lora_dev_para.band )
    	    {
    			if(tempChannel >= 166)
    	    	{
    	    		tempChannel = (tempChannel - 166) / 8 * 8 + 173;
    	    	}
    	    	else
    	    	{
    	    		tempChannel = tempChannel / 8 * 8 + 7;
    	    	}
    	    }
    		else
    		{
    			if(tempChannel >= 166)
    	    	{
    	    		tempChannel = (tempChannel - 166) / 8 * 8 + 107;
    	    	}
    	    	else
    	    	{
    	    		tempChannel = tempChannel / 8 * 8 + 75;
    	    	}
    		}
    	    frequency = CN470_FIRST_TX_CHANNEL + tempChannel *
    	    	CN470_STEPWIDTH_RX1_CHANNEL;
...
bool RegionCN470TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    *txPower = txPowerLimited;
	if(lora_dev_para.debug)
    printf("Tx, Power: %d,  Freq: %u,DR: %d, len: %u, duration %llu, at %llu\r\n",
                phyTxPower,  (unsigned int)Channels[txConfig->Channel].Frequency, txConfig->Datarate, txConfig->PktLen, *txTimeOnAir, curTime);
...
uint8_t RegionCN470LinkAdrReq( LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed )
{
    ...
    while( bytesProcessed < linkAdrReq->PayloadSize )
    {
        ...
        if( linkAdrParams.ChMaskCtrl == 6 )
        {
			...
			memcpy( channelsMask, ChannelsMask, sizeof(ChannelsMask) );
