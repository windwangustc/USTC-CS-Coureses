...
#if 0
typedef struct lora_heartbeat_para_s
{
    uint8_t  Beatflag;
    uint32_t Beattime;
}lora_heartbeat_para_t;
#endif

typedef struct lora_dev_para_s
{
    uint32_t baud;
    uint8_t power;
    uint8_t szlight;
    ...
    uint8_t  Beatflag;
    uint32_t Beattime;
    uint8_t Beat_startAddr;
    uint16_t Beat_endAddr;
    //uint8_t lorabeat_voltage[L_BEAT_LEN];
   // uint8_t lorabeat_current[L_BEAT_LEN];
    //uint8_t lorabeat_power[L_BEAT_LEN];
}lora_dev_para_t;

typedef struct lora_dev_gkey
{
   uint32_t devaddr;
   uint8_t appskey[16];
   uint8_t nwkskey[16];  
...
typedef struct lora_dev_group
{
	lora_dev_gkey_t lora_group[3];
	uint16_t flag;
}lora_dev_group;
