 ...
 */
static uint8_t TimerSetTimeout( TimerEvent_t *obj );
...
static void TimerInsertNewHeadTimer( TimerEvent_t *obj, uint32_t remainingTime )
{
    ...
    obj->IsRunning = true;
    TimerListHead = obj;
    /* stop the back grand timer, and adjust the system time */
...
static uint8_t TimerSetTimeout( TimerEvent_t *obj )
{
    ...
    if(obj->Timestamp > RTC_BORAD_MAX_INTERVAL)
    {
    	res = RtcSetTimeout( RTC_BORAD_MAX_INTERVAL );
    }
    else
    {
    	res = RtcSetTimeout( obj->Timestamp );
    }
    return res;
}
