...
uint32_t mulcast_addr = 0xFFFFFFFF;
uint8_t mulcast_appskey[16] = {0xdd,0x98,0x92,0x9b,0x92,0xf0,0x9e,0x2d,0xaf,0x67,0x6d,0x64,0x6d,0x0f,0xff,0xff};
uint8_t mulcast_nwkskey[16] = {0xdd,0x98,0x92,0x9b,0x92,0xf0,0x9e,0x2d,0xaf,0x67,0x6d,0x64,0x6d,0x0f,0xff,0xfe};
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
	...
	if(lora_dev_para.debug == 1)
	{
        for(int i = 0; i < size ; i++ )
		{
	       PRINTF_RAW("%02X-",payload[i]);
        }
    }
    ...
    switch ( macHdr.Bits.MType ) {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            {
                if( micRx == mic ) {
                    LoRaMacJoinComputeSKeys( LoRaMacAppKey, LoRaMacRxPayload + 1, LoRaMacDevNonce, g_lwan_dev_keys_p->abp.nwkskey, g_lwan_dev_keys_p->abp.appskey);
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN: {
            ...
            if ( address != LoRaMacDevAddr ) {
                ...
                {
					if ( address == lora_dev_para_g.lora_group[m].devaddr) 
					{
						 multicast = 1;
						 nwkSKey = lora_dev_para_g.lora_group[m].nwkskey;
						 appSKey = lora_dev_para_g.lora_group[m].appskey;
						 downLinkCounter = curMulticastParams->DownLinkCounter;
						 break;
					}
				}
				if(address == mulcast_addr)
				{
				    multicast = 1;
				    nwkSKey = mulcast_nwkskey;
					appSKey = mulcast_appskey;
					downLinkCounter = curMulticastParams->DownLinkCounter;
				}
                ...
                multicast = 0;
                nwkSKey = g_lwan_dev_keys_p->abp.nwkskey;
                appSKey = g_lwan_dev_keys_p->abp.appskey;
			...
			if(sequenceCounter >= sequenceCounterPrev ){
                sequenceCounterDiff = ( sequenceCounter - sequenceCounterPrev );
			}else{
				sequenceCounterDiff = (sequenceCounterPrev - sequenceCounter);
			}

            if(multicast == 1){
				downLinkCounter = 0;
				sequenceCounterDiff = sequenceCounter;
			}
            ...
            if(lora_dev_para.debug == 1){
			     printf("micRx:%u,%u,%u,%d\r\n",micRx,mic,downLinkCounter,address);
				 for(uint8_t k = 0; k< 16; k++)
				 {
                     printf("%02x",nwkSKey[k]);
				 }
				 printf("\r\n");
            }
            ...
            if ( isMicOk == true ) {
                ...
                if ( ( ( size - 4 ) - appPayloadStartIndex ) > 0 ) {
                    ...
                    if ( port == 0 ) {
						...
						AckTimeoutTimer.IsRunning = false;
                    }
                } else {
#ifdef CONFIG_LWAN
                    if ( fCtrl.Bits.Adr != AdrCtrlOn ) {
                ...
                if (lora_dev_para.debug) {
                    printf("MIC verify failed ignore the frame\r\n");
                }
    ...
    if ( LoRaMacFlags.Bits.MacDone == 1 ) {
        ...
        if ( ( NodeAckRequested == false ) && ( noTx == false ) ) {
            if ( ( LoRaMacFlags.Bits.MlmeReq == 1 ) || ( ( LoRaMacFlags.Bits.McpsReq == 1 ) ) ) {
                if ( ( LoRaMacFlags.Bits.MlmeReq == 1 ) && ( MlmeConfirm.MlmeRequest == MLME_JOIN ) ) {
                  ...
                  //  printf("again:%d,%d,%d",ChannelsNbRepCounter,LoRaMacParams.ChannelsNbRep,LoRaMacFlags.Bits.McpsInd);
        ...
        if ( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & LORAMAC_TX_DELAYED ) == 0 ) ) {
            ...
            if ( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) ) {
				...
				printf("Try to send the frame again\r\n");
...
static uint8_t ParseMacCommandsToRepeat( uint8_t *cmdBufIn, uint8_t length, uint8_t *cmdBufOut )
{
	...
	if(lora_dev_para.debug)
printf("LoRaMacParams.ChannelsDatarate in line2209 is %d\r\n",LoRaMacParams.ChannelsDatarate);
...
{
    if(lora_dev_para.debug)
	printf("Entering func ScheduleTx() now!\r\n");
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            if ( ( payload != NULL ) && ( LoRaMacTxPayloadLen > 0 ) ) {
                ...
                if ( framePort == 0 ) {
                    ...
                    MacCommandsBufferIndex = 0;
                    LoRaMacPayloadEncrypt( (uint8_t * ) payload, LoRaMacTxPayloadLen, g_lwan_dev_keys_p->abp.nwkskey, LoRaMacDevAddr, UP_LINK,
                                           UpLinkCounter, &LoRaMacBuffer[pktHeaderLen] );
                } else {
                    LoRaMacPayloadEncrypt( (uint8_t * ) payload, LoRaMacTxPayloadLen, g_lwan_dev_keys_p->abp.appskey, LoRaMacDevAddr, UP_LINK,
                                           UpLinkCounter, &LoRaMacBuffer[pktHeaderLen] );
                }
            }
            LoRaMacBufferPktLen = pktHeaderLen + LoRaMacTxPayloadLen;

            LoRaMacComputeMic( LoRaMacBuffer, LoRaMacBufferPktLen, g_lwan_dev_keys_p->abp.nwkskey, LoRaMacDevAddr, UP_LINK, UpLinkCounter, &mic );
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
	...
	memset(UartPayload,0xFF,sizeof(UartPayload));
	if (lora_dev_para.debug)
	{
...
{   
    ...
    switch ( mibSet->Type ) {
        ...
        case MIB_CHANNELS_DEFAULT_MASK: {
            chanMaskSet.ChannelsMaskIn = lora_dev_para.freqBand;
        ...
        case MIB_CHANNELS_DATARATE: {
             ...
             // verify.DatarateParams.Datarate =lora_dev_para.datarate;
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    switch ( mlmeRequest->Type ) {
        case MLME_JOIN: {
            ...
            ResetMacParameters( );
			//printf("data:%d",LoRaMacParams.ChannelsDatarate);
...
#endif
            printf("altDr.NbTrials:%d\r\n",altDr.NbTrials);
           LoRaMacParams.ChannelsDatarate = RegionAlternateDr( LoRaMacRegion, &altDr );
#ifdef CONFIG_LORA_VERIFY
            if (lora_dev_para.debug == true)
                PRINTF_RAW("MacHdr major:%d rfu:%d mtype:%d\r\n", macHdr.Bits.Major, macHdr.Bits.RFU, macHdr.Bits.MType);
#endif
...
{
    if(lora_dev_para.debug)
	printf("Entering func LoRaMacMcpsRequest() now!\r\n");
    GetPhyParams_t getPhy;
    PhyParam_t phyParam;
    LoRaMacStatus_t status = LORAMAC_STATUS_SERVICE_UNKNOWN;
    LoRaMacHeader_t macHdr;
